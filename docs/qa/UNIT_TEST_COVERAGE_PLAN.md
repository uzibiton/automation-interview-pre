...
- Unit tests run from the test project at `tests/config`.
...
- `tests/unit/stores/useInvitationStore.test.ts`
- Template: `tests/unit/services/expenses.service.test.ts.template`

### Test infrastructure

- Jest config and runner: `tests/config/jest.config.js`, `tests/config/package.json`.
- Playwright E2E config: `tests/config/playwright.config.ts`.
- Coverage artifacts folder: `coverage` (current reports show 0%).

### Known constraints

- Root `package.json` does not include `tests/config` as a workspace; running root `npm run test:unit` may fail unless executed from `tests/config`.
...
- `AuthService` methods
- `ExpensesService` query/filter logic
- Authorization checks in `GroupsService`
...
- What NOT to test in CI unit pipeline: large-scale load tests.
...
Security vulnerabilities, data integrity issues, and privilege escalation risks live in these untested areas; increasing coverage reduces regression risk and improves release confidence.
...
- **Unit**: run via Jest in `tests/config` with mocks.
...
- **Integration/E2E**: use seeded real DB and services; seed scripts: run `npm run seed:enhanced` or `npm run seed:postgres`.
...
```bash
# Run unit tests with coverage from test project
cd tests/config
...
1. Add handoff file `docs/qa/UNIT_TEST_COVERAGE_PLAN.md`.
2. Implement unit tests for `AuthService` first (security-critical).
3. Implement `ExpensesService` unit tests next (business-critical).
4. Implement `GroupsService` tests (authorization).
5. Run coverage and iterate thresholds; then implement Integration/E2E as needed.
...
- **DO NOT** test TypeORM or Firestore internals; test service behavior.

## 10. Open Questions

- Should CI run a matrix for both `DATABASE_TYPE=postgres` and `DATABASE_TYPE=firestore` to cover both branches?
- Is the team willing to add `tests/config` into root `package.json` workspaces, or should tests continue to be run from inside `tests/config`?
